    ...
    if (cartItems.length === 0) {
        return (
            <section className='flex justify-center items-center flex-col gap-y-1 h-screen w-full relative'>
                ...
                <h3 className='text-center text-6xl dancing_script'>No Items In Cart</h3>
                <Link href='/' className='mt-5'>
                    <ButtonTextIcon Text='Continue Shopping' Icon={<i className="ri-arrow-left-line"></i>} customize="px-4 py-2 transition-all duration-1000 w-full hover:rounded-full" />
                </Link>
            </section>
        )
    }
        ...
        <>
            ...
            <section className='min-h-screen w-full relative'>
                ...
                <div className='mt-[20vh] md:mt-[25vh] w-[90%] mx-auto'>
                        ...
                        isAuthenticated && (
                            <div className='flex items-center justify-center md:justify-start relative gap-x-6 mb-14 md:mb-10'>
                                <div className="size-20 relative rounded-full overflow-hidden border border_color">
                                    <Image fill src={user?.avatar?.url} alt={user.name} className='h-full w-full object-cover' />
                                </div>
                    ...
                    <div>
                        {
                            cartItems && cartItems.map((val) => <CartItem key={val.id} {...val} />)
                        }
                    </div>
                    ...
                    <div className='flex justify-between border-t border_color pt-8 items-center flex-col lg:flex-row'>
                        ...
                        <div onClick={clearItems}>
                            <ButtonTextIcon Text='Clear All Items' Icon={<i className="ri-delete-bin-7-line"></i>} customize='px-4 py-2 transition-all duration-1000 text-red-600 border border-red-600 hover:rounded-full' />
                        </div>
                    </div>
                    ...
                    <div className='mt-20'>
                        <div className="w-fit border border_color mx-auto md:ml-auto md:mr-0 space-y-3.5 p-3">
                            <p className='darker_grotesque font-semibold text-xl'>SubTotal: <span className='obviously text-sm'>{cartItems && <FormatPrice price={subtotal} />}</span></p>
                            <p className='darker_grotesque font-semibold text-xl'>Shipping Fee: <span className='obviously text-sm'><FormatPrice price={shippingCharges} /> </span></p>
                            <p className='darker_grotesque font-semibold text-xl'>Order Total: <span className='obviously text-sm'><FormatPrice price={subtotal + shippingCharges} /> </span></p>
                            <div onClick={checkoutHandler}>
                                <ButtonTextIcon Text='Checkout' Icon={<ShoppingCartCheckoutOutlinedIcon />} customize='px-3 py-1.5 transition-all duration-1000 hover:rounded-full' />
                            </div>
                        </div>
                    </div>
