...
import { clearErrors, deleteUser, getAllUsers } from "@/store/actions/userAction";
...
const UsersList = () => {
    ...
    const { loading, error: deleteError, isDeleted, message } = useSelector((state) => state.profile);
    ...
    const columns = [
...

        {
            ...
            flex: 0.5,
            renderCell: (params) => {
                return <Image width={40} height={40} className="rounded-full" src={params.row.image} alt="profile" />
            },
        },
            ...
            {loading ? <Loader /> :
                <section className="flex w-screen">
                    ...
                    <div className="px-10 pt-20 pb-10">
                        <h1 className="text-3xl text-center mb-7">ALL USERS ({users?.length}) </h1>
                        <DataGrid
                            rows={rows}
                            columns={columns}
                            pageSize={7}
